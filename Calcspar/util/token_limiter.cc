...
void TokenLimiter::RequestToken(Env::IOSource io_src, IOType io_type,
  ...
  if(!updated&&now_us-refill_us>=T_middle)
  {
    ...
    {
      int t=R_2-R_1;
      int iops=Monitor::GetAll();
      if(iops<tokens_per_sec_*0.850&&t<tokens_per_sec_*0.050)
      {
        t+=tokens_per_sec_*0.050;
      }
      else if(iops>tokens_per_sec_*0.850&&t>tokens_per_sec_*0.050)
      {
        t-=tokens_per_sec_*0.025;
      }
      ...
      T_low=std::max((uint64_t)0,T_low);
      T_middle=std::max((uint64_t)0,T_middle);
